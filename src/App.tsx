...
export default function App() {
  ...
  const [scopeProjects, setScopeProjects] = useState<"active" | "archive">(
    ((localStorage.getItem(LS.scopeProjects) as "active" | "archive") ||
      "active")
  );
  const [scopePersons, setScopePersons] = useState<"active" | "archive">(
    ((localStorage.getItem(LS.scopePersons) as "active" | "archive") ||
      "active")
  );
  const [scopeTasks, setScopeTasks] = useState<"active" | "archive">(
    ((localStorage.getItem(LS.scopeTasks) as "active" | "archive") ||
      "active")
  );
  ...
  useEffect(() => {
    localStorage.setItem(LS.taskView, taskView);
  }, [taskView]);
  useEffect(() => {
    localStorage.setItem(LS.scopeProjects, scopeProjects);
  }, [scopeProjects]);
  useEffect(() => {
    localStorage.setItem(LS.scopePersons, scopePersons);
  }, [scopePersons]);
  useEffect(() => {
    localStorage.setItem(LS.scopeTasks, scopeTasks);
  }, [scopeTasks]);
            ...
            <>
                    ...
                    <div className="ml-auto flex items-center gap-2">
                      ...
                      <button
                        className={`rounded-xl border px-3 py-1 ${
                          taskView === "board" ? "bg-muted/50" : ""
                        }`}
                        onClick={() => setTaskView("board")}
                        disabled={scopeTasks === "archive"}
                        title={
                          scopeTasks === "archive"
                            ? "Канбан доступен только для активных задач"
                            : ""
                        }
                      >
                        Канбан
              ...
              {tab === "persons" && (
                <div className="space-y-4">
                  <ScopeTabs
                    scope={scopePersons}
                    onChange={setScopePersons}
                  />
                  <EntityTable
                    entity="person"
                    archived={scopePersons === "archive"}
                  />
                </div>
              )}
              ...
              <tr key={fd._id} className="border-t">
                ...
                <td className="p-2 text-right">
                  <button
                    ...
                    aria-label="Удалить"
                    onClick={async () => {
                      if (!confirm(`Удалить поле «${fd.label || fd.key}»?`))
                        return;
                      await api(`/api/fields/${fd.entity}/${fd.key}`, {
                        method: "DELETE",
                      });
